<?php
    ...
    if(isset($_POST['generel'])){
        ...
        if(empty($for_navn) || empty($efter_navn)|| empty($email) || empty($post_nr) || empty($by_navn)){
            header("Location: registrer.php?error=emptyfields&fornavn=".$fornavn."&efternavn=".$efternavn."&email=".$email."&postnr=".$postnr."&by=".$by_navn."&telenr=".$telenr);
            exit();
        }elseif(!preg_match("/^[a-zA-Z]*$/", $for_navn)){
            header("Location: registrer.php?error=invalidFornavn&efternavn=".$efternavn."&email=".$email."&postnr=".$postnr."&by=".$by_navn."&telenr=".$telenr);
            exit();
        }elseif(!preg_match("/^[a-zA-Z]*$/", $efter_navn)){
            header("Location: registrer.php?error=invalidEfternavn&fornavn=".$fornavn."&email=".$email."&postnr=".$postnr."&by=".$by_navn."&telenr=".$telenr);
            exit();
        }elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)){
            header("Location: registrer.php?error=invalidmail&fornavn=".$fornavn."&efternavn=".$efternavn."&postnr=".$postnr."&by=".$by_navn."&telenr=".$telenr);
            exit();
        }elseif(!preg_match("/^[0-9]*$/", $post_nr)){
            header("Location: registrer.php?error=invalidPostnr&fornavn=".$fornavn."&efternavn=".$efternavn."&email=".$email."&by=".$by_navn."&telenr=".$telenr);
            exit(); 
        }elseif(!preg_match("/^[a-zA-Z]*$/", $by_navn)){
            header("Location: registrer.php?error=invalidBy&fornavn=".$fornavn."&efternavn=".$efternavn."&email=".$email."&postnr=".$postnr."&telenr=".$telenr);
            exit();
        }else{                 
            $sql = "UPDATE user_oplysninger SET for_navn='$for_navn', efter_navn='$efter_navn', email='$email', post_nr='$post_nr', by_navn='$by_navn' WHERE user_name='$user_name'";
    ...
    if(isset($_POST['kodeord'])){
        ...
        print_r($pass_word.",".$gentag_password);
    }
?>
...
<div class="container <?php if(!isset($_SESSION['adgang'])) echo ' hidden'; ?>">
    ...
    <div class="form-group">
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" onSubmit="return checkform()" id="checkform">
            <div class="form-group">
                ...
                <input type="hidden" value="$login_session" name="username">
                <div class="row">
    ...
    <div class="form-group">   
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" onSubmit="return" id="billede">
            ...
            <button class="btn" name="billede"><h4>Opdater billede</h4></button>
        </form>
    </div>
</div>
